...
namespace AnimationInstancing
{
    public class AnimationGenerator : EditorWindow
    {
        ...
        {
            string folderPath = "Assets/MyTest/Resources/RoleRelated/Roles/Prefabs"; // 替换为你的目标文件夹路径
            ...
            foreach (string guid in guids)
            {
                ...
                if (!prefab.TryGetComponent<AnimationInstancing>(out var ai))
                {
                    prefab.AddComponent<AnimationInstancing>();
                    // 更新Prefab
        ...
        private void Process(GameObject prefab)
        {
            ...
            SkinnedMeshRenderer[] meshRender1 = generatedPrefab.GetComponentsInChildren<SkinnedMeshRenderer>();
            List<Matrix4x4> bindPose1 = new List<Matrix4x4>(150);
            boneTransform = RuntimeHelper.MergeBone(meshRender1, bindPose1);
            ...
            if (generatedPrefab)
            {
                ...
                if (animator == null)
                {
                    Debug.LogError("Error: The prefab should have a Animator Component.");
                    return;
                }
                if (animator.runtimeAnimatorController == null)
                {
                    Debug.LogError("Error: The prefab's Animator should have a Animator Controller.");
                    return;
                }
                ...
                if (textureCount == 0)
                    Debug.LogError("Error: There is certain animation's frames which is larger than a whole texture.");
                else if (textureCount == 1)
                    Debug.LogError(string.Format("Animation Texture will be one {0} X {1} texture", textureWidth, textureWidth));
                else
                    Debug.LogError(string.Format("Animation Texture will be {2} 1024 X 1024 and one {0} X {1} textures", textureWidth, textureWidth, textureCount - 1));
                ...
                foreach (var clipName in clipNames)
                {
                    ...
                    generateAnims[clipName] = true;

                    if (framesToBake > 5000)
                    {
                        Debug.LogError("<color=red>Long animations degrade performance, consider using a higher frame skip value.</color>");
                    }
                }
            }
        ...
        private void OnGUI()
        {
            // if (GUILayout.Button(string.Format("Generate All")))
            // {
            //     GenerateAll();
            // }
            // return;
            ...
            {
                UnityEngine.Debug.Log("prefab != generatedPrefab");
                generateAnims.Clear();
                customClips.Clear();
                generatedPrefab = prefab;
